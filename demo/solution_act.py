...
class AlgSolution:
    ...
    def __init__(self):
        ...
        default_ckpt = os.path.join(repo_root, "atec_robot_model", "baseline", "act", "policy.pt")
        ckpt_path = os.environ.get("ATEC_ACT_POLICY_CKPT", default_ckpt)
        if not os.path.isfile(ckpt_path):
            raise FileNotFoundError(
                f"ACT checkpoint not found: {ckpt_path}. "
                "Unpack atec_robot_model.zip from readme.md, or set ATEC_ACT_POLICY_CKPT to a .pt file."
            )
