...
def randomize_arm_joint_targets(
    ...
    """Randomize arm joint position targets to simulate arm perturbation during locomotion.

    This function adds random noise to arm joint position targets. The PD controllers
    will attempt to track the perturbed targets, causing the arm to move. This forces
    the locomotion policy to be robust to arm movements and mass shifts.

    Args:
        env: The simulation environment.
        env_ids: Specific environment indices to apply randomization.
        asset_cfg: Scene entity config targeting the robot.
        amount_range: Range for random perturbation (min, max) in radians.
    ...
    if distribution == "uniform":
        noise = torch.FloatTensor(n_envs, n_joints).uniform_(*amount_range).to(env.device)
    elif distribution == "gaussian":
        noise = torch.FloatTensor(n_envs, n_joints).normal_(mean=0.0, std=amount_range[1]).to(env.device)
